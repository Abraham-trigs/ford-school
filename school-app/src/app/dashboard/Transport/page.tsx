...
export default function TransportPage() {
  ...
  const [recordToDelete, setRecordToDelete] = useState<TransportRecord | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    axios.get("/api/transport").then((res) => setRecords(res.data));
  }, []);
  ...
  const handleSuccess = (record: TransportRecord) => {
    setRecords((prev) => {
  ...
  const handleDeleteClick = (record: TransportRecord) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    setLoading(true);
    try {
      await axios.delete(`/api/transport?id=${recordToDelete.id}`);
    ...
    { header: "Capacity", accessor: "capacity" },
    {
      ...
      render: (record: TransportRecord) => (
        <div className="flex space-x-2">
          ...
          {["ADMIN", "TRANSPORT"].includes(user?.role || "") && (
            <button
  ...
  return (
    <div className="p-6 bg-background min-h-full rounded-lg shadow">
      ...
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-2xl font-display text-primary">Transport</h1>
        {["ADMIN", "TRANSPORT"].includes(user.role) && (
          <button
      ...
      <DeleteModal
        ...
        message={`Delete "${recordToDelete?.vehicleNumber}"?`}
      />
