...
import ParentModal from "@/components/Parents/ParentModal";
import DeleteModal from "@/components/common/DeleteModal";
import LoaderModal from "@/components/layout/LoaderModal";

export interface ParentAlumni {
  id?: string;
  name: string;
  relation: string;
  studentName?: string;
...
export default function ParentsPage() {
  const [records, setRecords] = useState<ParentAlumni[]>([]);
  const [modalOpen, setModalOpen] = useState(false);
  const [recordToEdit, setRecordToEdit] = useState<ParentAlumni | undefined>();
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [recordToDelete, setRecordToDelete] = useState<ParentAlumni | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    axios.get("/api/parents").then((res) => setRecords(res.data));
  }, []);

  const handleCreate = () => setModalOpen(true);
  const handleEdit = (record: ParentAlumni) => {
    setRecordToEdit(record);
    setModalOpen(true);
  };
  const handleSuccess = (record: ParentAlumni) => {
    setRecords((prev) => {
  ...
  const handleDeleteClick = (record: ParentAlumni) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    setLoading(true);
    try {
      await axios.delete(`/api/parents?id=${recordToDelete.id}`);
  ...
  return (
    <div className="p-6 bg-background min-h-full rounded-lg shadow">
      ...
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-2xl font-display text-primary">Parents / Alumni</h1>
        {["ADMIN", "PRINCIPAL"].includes(user.role) && (
          <button
