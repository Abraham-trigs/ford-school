...
export default function Sidebar({
  ...
  return (
    <motion.aside
      ...
      className={`bg-purple text-deepest0 h-screen flex flex-col relative ${paddingClass}`}
    >
      {/* Desktop Collapse Button */}
      {!isMobile && (
        <button
          onClick={toggleSidebar}
          className="absolute -right-3 top-6 bg-greener text-deepest0 hover:text-lightGray hover:border-greener border-2 border-deeper p-1 rounded-full hover:bg-deeper ease-in-out transition-colors z-50"
        >
          {sidebarCollapsed ? (
      ...
      {/* Logo / Title */}
      <div className="text-xl font-display font-bold mb-8 whitespace-nowrap">
        {!isMobile && !sidebarCollapsed && `${fullName}`}
      </div>
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-3 px-3 py-2 rounded-lg transition-colors duration-200 ${
                isActive
                  ? "bg-deepest text-purple0 font-semibold"
                  : "hover:bg-deepest hover:text-purple"
