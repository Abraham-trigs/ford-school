...
import ITModal from "@/components/IT/ITModal";
...
export default function ITSupportPage() {
  const [records, setRecords] = useState<ITSupport[]>([]);
  const [modalOpen, setModalOpen] = useState(false);
  const [recordToEdit, setRecordToEdit] = useState<ITSupport | undefined>();
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [recordToDelete, setRecordToDelete] = useState<ITSupport | null>(null);
  const [user, setUser] = useState<{ role: string } | null>(null);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    axios.get("/api/auth/me").then((res) => setUser(res.data.user));
    axios.get("/api/it").then((res) => setRecords(res.data));
  }, []);

  const handleCreate = () => setModalOpen(true);
  const handleEdit = (record: ITSupport) => {
    setRecordToEdit(record);
    setModalOpen(true);
  };
  const handleSuccess = (record: ITSupport) => {
    setRecords((prev) => {
  ...
  const handleDeleteClick = (record: ITSupport) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    setLoading(true);
    try {
      await axios.delete(`/api/it?id=${recordToDelete.id}`);
  ...
  return (
    <div className="p-6 bg-background min-h-full rounded-lg shadow">
      ...
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-2xl font-display text-primary">IT Support</h1>
        {["ADMIN", "IT_SUPPORT"].includes(user.role) && (
          <button
