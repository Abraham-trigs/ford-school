...
export default function GuestPage() {
  ...
  const [recordToEdit, setRecordToEdit] = useState<Guest | undefined>();
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [recordToDelete, setRecordToDelete] = useState<Guest | null>(null);
  const [user, setUser] = useState<{ role: string } | null>(null);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    axios.get("/api/auth/me").then((res) => setUser(res.data.user));
    axios.get("/api/guests").then((res) => setRecords(res.data));
  }, []);

  const handleCreate = () => setModalOpen(true);
  const handleEdit = (record: Guest) => {
    setRecordToEdit(record);
    setModalOpen(true);
  };
  const handleSuccess = (record: Guest) => {
    setRecords((prev) => {
  ...
  const handleDeleteClick = (record: Guest) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    setLoading(true);
    try {
      await axios.delete(`/api/guests?id=${recordToDelete.id}`);
  ...
  return (
    <div className="p-6 bg-background min-h-full rounded-lg shadow">
      ...
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-2xl font-display text-primary">Guests</h1>
        {["ADMIN", "GUEST"].includes(user.role) && (
          <button
