...
import NurseModal from "@/components/Nurses/NurseModal";
...
export default function NursesPage() {
  ...
  const [recordToEdit, setRecordToEdit] = useState<Nurse | undefined>();
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [recordToDelete, setRecordToDelete] = useState<Nurse | null>(null);
  const [user, setUser] = useState<{ role: string } | null>(null);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    axios.get("/api/auth/me").then((res) => setUser(res.data.user));
    axios.get("/api/nurses").then((res) => setRecords(res.data));
  }, []);
  ...
  const handleSuccess = (record: Nurse) => {
    setRecords((prev) => {
  ...
  const handleDeleteClick = (record: Nurse) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    setLoading(true);
    try {
      await axios.delete(`/api/nurses?id=${recordToDelete.id}`);
  ...
  return (
    <div className="p-6 bg-background min-h-full rounded-lg shadow">
      ...
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-2xl font-display text-primary">Nurses</h1>
        {["ADMIN", "NURSE"].includes(user.role) && (
          <button
