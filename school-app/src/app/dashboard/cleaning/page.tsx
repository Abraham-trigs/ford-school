...
import CleaningStaffModal from "@/components/Cleaning/CleaningStaff";
import DeleteModal from "@/components/common/DeleteModal";
import LoaderModal from "@/components/common/LoaderModal";
import { CleaningStaff } from "@/types/cleaningStaff";

export default function CleaningStaffPage() {
  const [records, setRecords] = useState<CleaningStaff[]>([]);
  const [modalOpen, setModalOpen] = useState(false);
  const [recordToEdit, setRecordToEdit] = useState<CleaningStaff | undefined>();
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [recordToDelete, setRecordToDelete] = useState<CleaningStaff | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    const fetchRecords = async () => {
      setLoading(true);
      try {
        const res = await axios.get("/api/cleaning");
        setRecords(res.data);
      } catch (err: any) {
        toast.error(
          err.response?.data?.message || "Failed to fetch Cleaning Staff"
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleEdit = (record: CleaningStaff) => {
    setRecordToEdit(record);
    setModalOpen(true);
  };

  const handleSuccess = (record: CleaningStaff) => {
    setRecords((prev) => {
  ...
  const handleDeleteClick = (record: CleaningStaff) => {
  ...
  return (
    <div className="p-6 bg-background min-h-full rounded-lg shadow">
      ...
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-2xl font-display text-primary">Cleaning Staff</h1>
        {["ADMIN", "PRINCIPAL"].includes(user.role) && (
          <button
