...
export async function GET(
  ...
  try {
    ...
    if (!roles.includes("SUPERADMIN")) {
      const requesterMemberships = await prisma.userSchoolSession
        .findMany({ where: { userId: requesterId, active: true } })
        .then((ms) => ms.map((m) => m.schoolSessionId));
      const userMemberships = user.memberships.map((m) => m.schoolSessionId);
      if (!canManageUser(roles, requesterMemberships, userMemberships))
...
export async function PUT(
  ...
  try {
    ...
    const safeProfileData = profileData
      ? filterProfileData(role, profileData)
      : undefined;
      ...
      data: {
        ...
        memberships: schoolSessionId
          ? {
              upsert: {
                where: {
                  userId_schoolSessionId: { userId, schoolSessionId },
                },
                update: {
                  email: email ?? undefined,
                ...
                create: {
                  email,
                  password:
                    hashedPassword ?? crypto.randomBytes(12).toString("hex"),
                  role,
                  schoolSessionId,
                  userId,
                  active: active ?? true,
          ...
          : undefined,
        studentProfile:
          role === "STUDENT" && safeProfileData
            ? {
                upsert: {
                  where: { userId },
                  update: safeProfileData,
                  create: safeProfileData,
                },
              }
            : undefined,
        teacherProfile:
          ["TEACHER", "PRINCIPAL", "VICE_PRINCIPAL"].includes(role) &&
          safeProfileData
            ? {
                upsert: {
                  where: { userId },
    ...
    updatedUser.memberships = updatedUser.memberships.map((m) => ({
      ...m,
      password: undefined,
    }));

    console.log(
      `[AUDIT] User ${requesterId} updated user ${userId} at ${new Date().toISOString()}`
    );
...
export async function DELETE(
  ...
  try {
    ...
    console.log(
      `[AUDIT] User ${requesterId} soft-deleted user ${userId} at ${new Date().toISOString()}`
    );
