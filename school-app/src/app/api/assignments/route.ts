...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!roles.includes("SUPERADMIN")) {
      ...
      if (roles.includes("ADMIN")) whereClause.course = { schoolSessionId: { in: allowedSchoolIds } };
      else if (roles.includes("TEACHER")) whereClause.course = { teacherId: userId };
    }
...
export async function POST(req: NextRequest) {
  try {
    const { roles, userId } = await verifyToken(req);
    if (!["SUPERADMIN", "ADMIN", "TEACHER"].some(r => roles.includes(r))) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
    ...
    if (!(await canManageAssignment(userId, roles, courseId))) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
